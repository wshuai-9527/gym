import{NORMALIZE_RULES,TYPE_META}from'./data.js?v=16';
...
export function parseRawRecords(raw){return raw.trim().split(/\n\s*\n/).map(block=>{const lines=block.split('\n').map(x=>x.trim()).filter(Boolean);const date=lines.shift();const items=[];for(let i=0;i<lines.length;i++){if(!/^\d/.test(lines[i])||lines[i]==='🏀'){const name=canon(lines[i]);const sets=[];while(lines[i+1]&&/^(\d|[\d.]+kg|[\d.]+s)/.test(lines[i+1]))sets.push(lines[++i].replaceAll('×','*'));items.push({name,sets})}}return normalizeRecord({date,items})}).filter(r=>r.date)}
...
export function mergeRecords(current,defaults){const map=new Map();normalizeRecords(defaults).forEach(r=>map.set(r.date,r));normalizeRecords(current).forEach(r=>map.set(r.date,r));return[...map.values()].sort((a,b)=>cmpMD(a.date,b.date))}
